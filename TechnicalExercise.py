'''
Title: Machine Learning Model for a manufacturing process dataset
Author: Lívia Parente
Date: 01/02/2019
'''
import csv
import numpy as np
...
def plot_ROC(y_true, y_score):
	#Plot ROC curve and calculates AUC
...
secom.dtypes 
# Print shape
print("Shape of the entire dataset:", secom.shape)
secom.info()
...
# Replace missing values (NaN and 0.0)
for column in X:
	X[column].replace(0.0, np.nan, inplace= True) 					# Replace nul values for NaN
	mean = X[column].mean()											# Calculates mean of column
	if np.isnan(mean):												# Erases NaN columns
		X = X.drop([column],axis=1)
	else:															# Replaces NaN values with the mean
...
pipeline = make_pipeline(preprocessing.StandardScaler(), 				#Pipeline that first fits the data by Calling the transformer API 
                         RandomForestClassifier(class_weight='balanced', random_state=123, n_estimators=50))		#and then models it throught RandomForestClassifier
...
#########################################################################################################################################################
# Selecting features by importances
print("\n Feature selection:")
...
print("Accuracy after feature selection:",metrics.accuracy_score(y_test, y_pred))
